...
def avancer2(distance, vitesse, ser):
    """
    Fonction pour envoyer une commande pour avancer le robot.
    ...
    Returns:
        tuple:
            rien remember to fluch reeinnit after
    """
    ...
    if len(vitesse) != 3:
        ...
        v = vitesse
    s.send_msg_4bits(code_function + d + v, ser)
    return True
...
def reculer2(distance, vitesse, ser):
    """
    Fonction pour envoyer une commande pour reculer le robot.
...
def cibler2(x, y, vitesse, ser):
    ...
    s.send_msg_4bits(code_function + x + y + v, ser)
    return True
...
def rejoindre2(x, y, sens, vitesse, ser):
    """
    Fonction pour envoyer une commande pour rejoindre une position avec un sens spécifié.
    ...
    if len(sens) != 1:
        return False
    s.send_msg_4bits(code_function + x + y + sens + vitesse, ser)
    return erreur_Evitement, erreur_Blockage
...
def go_to(x, y, sens, vitesse, ser):
    cib=cibler(x,y,vitesse,ser)      
    rej=rejoindre(x, y, sens, vitesse, ser)
    return cib,rej

def go_to2(x, y, sens, vitesse, ser):
    cib=cibler2(x,y,"100",ser)
    s.timer_s(2)    
    rej=rejoindre2(x, y, sens, vitesse, ser)
    return cib,rej

def go_to_evitement(x, y, sens, vitesse, ser):
    cib=cibler(x,y,vitesse,ser)
    if cib[1]==False:
        rej=rejoindre(x, y, sens, vitesse, ser)
        if rej[1]==True:
            
           go_to("1500", "1000", sens, vitesse, ser)
           go_to(x, y, sens, vitesse, ser)
    else:
         go_to("1500", "1000", sens, vitesse, ser)
...
def Evitement_on_off(state,ser):
